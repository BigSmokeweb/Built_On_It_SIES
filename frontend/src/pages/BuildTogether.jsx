...
export default function Collab() {
  ...
  return (
    <div style={{ background:'var(--bg)', minHeight:'100vh' }}>
      ...
      <div style={{ position:'sticky', top:0, zIndex:100 }}>
        <nav style={{ height: navOpen ? 57 : 0, overflow:'hidden', background:'rgba(13,17,23,.95)', borderBottom: navOpen ? '1px solid var(--border)' : 'none', display:'flex', alignItems:'center', padding: navOpen ? '0 clamp(12px,3vw,20px)' : '0', gap:10, backdropFilter:'blur(12px)', transition:'height .35s cubic-bezier(.4,0,.2,1), padding .35s', animation:'slideDown .4s ease' }}>
          ...
          <div style={{ marginLeft:'auto', display:'flex', gap:8, alignItems:'center', flexShrink:0 }}>
            <button className="btn btn-primary btn-sm" onClick={() => setShowPost(true)} style={{ whiteSpace:'nowrap' }}>+ Post Project</button>
      ...
      <div style={{ maxWidth:1160, margin:'0 auto', padding:'clamp(16px,3vw,28px) clamp(14px,3vw,20px)', display:'grid', gridTemplateColumns:'minmax(0,1fr)', gap:22, alignItems:'start' }}
        ...
        <div>
          ...
          {/* Stats strip */}
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:12, marginBottom:24, animation:'fadeIn .4s ease .05s both' }}>
      ...
      <style>{`
        @media(min-width:769px){.collab-grid{grid-template-columns:minmax(0,1fr) 290px!important}}
      `}</style>
    </div>
  );
}
